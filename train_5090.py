"""
Autoresearch pretraining script - Optimized for RTX 5090 (Blackwell)
Single-GPU, single-file. Using PyTorch Native SDPA for compatibility.
...
# Blackwell(RTX 5090) 호환성을 위해 외부 flash-attn3 라이브러리 의존성을 제거했습니다.
# 대신 PyTorch 2.9+의 SDPA와 torch.compile이 Blackwell 최적화 커널을 사용합니다.
...
class MuonAdamW(torch.optim.Optimizer):
    ...
    def _step_muon(self, group):
        ...
        p, state = params[0], self.state[params[0]]
        num_params, shape, device, dtype = len(params), p.shape, p.device, p.dtype
        if "momentum_buffer" not in state:
            state["momentum_buffer"] = torch.zeros(num_params, *shape, dtype=dtype, device=device)
        if "second_momentum_buffer" not in state:
            s_shape = (num_params, shape[-2], 1) if shape[-2] >= shape[-1] else (num_params, 1, shape[-1])
            state["second_momentum_buffer"] = torch.zeros(s_shape, dtype=dtype, device=device)
...
# Blackwell RTX 5090 Peak FLOPs 근사치 (학습 지표용)
BLACKWELL_BF16_PEAK_FLOPS = 1200e12 
...
# Blackwell에서 torch.compile은 선택이 아닌 필수입니다.
...
def get_lr_multiplier(progress):
    ...
    elif progress < 1.0 - WARMDOWN_RATIO: return 1.0
    else:
...
smooth_train_loss, total_training_time, step = 0, 0, 0

print(f"Starting training on RTX 5090 (Blackwell)... Budget: {TIME_BUDGET}s")
